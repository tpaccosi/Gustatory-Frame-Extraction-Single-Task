...
"""The script will extract the label list from the data itself. Please be sure your data and labels are clean.
  3 Labels: ['Taste_Word', 'Taste_Source', 'Quality']
  10 Labels: ['Taste_Word', 'Taste_Source', 'Quality', 'Location', 'Taste_Carrier', 'Evoked_Taste', 'Taste_Modifier', 'Effect', 'Taster', 'Circumstances']"""

def read_split_fold(split='train', fold="0", lang="english", label_dict=None):
    # Change the path template as needed.
    path = 'splits/data_{}/folds_{}_{}.tsv'.format(lang, fold, split)
    try:
...
def main():
    ...
    if language not in ['english']:
        raise Exception(f"Language error: {language} is not among the languages.")
    ...
    if args.hypsearch:
        # Hyperparameter search with compute_metrics: default maximization is through the sum of all the metrics returned
